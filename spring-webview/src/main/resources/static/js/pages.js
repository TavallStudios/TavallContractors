...
function projectFields(projects) {
...
function renderPortfolioPage(userId, container) {
    return MarketplaceApi.freelancerPortfolio(userId).then(view => {
        const portfolio = normalizePortfolio(view.portfolio);
        const skills = portfolio.skills.map(skill => `<span class="badge badge-blue">${escapeHtml(skill)}</span>`).join("");
      ...
      `;
        ...
        const links = portfolio.links.map(link => `<a class="btn btn-ghost btn-sm" href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(link.label)}</a>`).join("");
        container.innerHTML = `
...
async function renderFreelancerWorkspace(container) {
    ...
    `;
        ...
        saveButton?.addEventListener("click", async () => {
            ...
            const displayName = byId("workspace-display-name")?.value ?? "";
            const updatedHeadline = byId("workspace-headline")?.value ?? "";
            const rateValue = Number(byId("workspace-rate")?.value ?? 0);
            const location = byId("workspace-location")?.value ?? "";
            const availability = byId("workspace-availability")?.value ?? "";
            const tagline = byId("workspace-tagline")?.value ?? "";
            const summary = byId("workspace-summary")?.value ?? "";
            const updatedSkills = parseSkills(byId("workspace-skills")?.value ?? "");
            const projects = [0, 1, 2].map(index => ({
                title: byId(`project-title-${index}`)?.value ?? "",
                tech: byId(`project-tech-${index}`)?.value ?? "",
                imageUrl: byId(`project-image-${index}`)?.value ?? "",
                summary: byId(`project-summary-${index}`)?.value ?? ""
...
export async function hydratePage(page) {
    if (page === "hire-direct") {
        const container = byId("hire-direct-list");
        if (!container)
            return;
        const people = await MarketplaceApi.hireDirect();
        container.innerHTML = people.map(person => `<article class="card scroll-reveal">
    ...
    if (page === "freelancer") {
        ...
        container.innerHTML = jobs.map(job => `<article class="card scroll-reveal">
