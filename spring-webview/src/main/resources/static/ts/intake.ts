...
import { CartAddon, IntakeState, WizardSessionState } from "./types";

const SESSION_STORAGE_KEY = "tavall:intake:session";
const ADDON_CATALOG: CartAddon[] = [
  {
...
function initialIntake(): IntakeState {
  return {
    domain: "",
    domainType: "",
    domainSubtype: "",
...
function text(target: HTMLElement | null, value: string): void {
  if (target) target.textContent = value;
}

function estimateBudget(value: IntakeState): number {
  const budget = value.budget ?? "";
  if (budget === "1000-3000") return 2000;
  if (budget === "3000-10000") return 6500;
  if (budget === "10000+") return 15000;
  if (budget === "custom") return Number((value.customBudget ?? "").replace(/[^0-9.]/g, "")) || 0;
  if (budget === "no_budget" || budget === "no_budget_quote") return 0;
...
export async function wireIntake(): Promise<void> {
  ...
  const openButtons = Array.from(document.querySelectorAll<HTMLElement>("[data-intake-open]"));
  const closeButtons = Array.from(document.querySelectorAll<HTMLElement>("[data-intake-close]"));
  ...
  const addonCheckboxes = Array.from(document.querySelectorAll<HTMLInputElement>("[data-addon-code]"));

  let sessionState: WizardSessionState;
  try {
  ...
  function hydrateFormFromState(): void {
    ...
    formEl.querySelectorAll<HTMLInputElement>('input[name="docsType"]').forEach(input => {
  ...
  function selectedAddonsFromUI(): CartAddon[] {
    const selectedCodes = new Set(
      addonCheckboxes.filter(box => box.checked).map(box => box.dataset.addonCode ?? "")
    );
  ...
  function readCurrentIntake(): IntakeState {
  ...
  async function recomputeFromServer(): Promise<void> {
  ...
  cartItemsEl?.addEventListener("click", async event => {
    const target = event.target as HTMLElement | null;
    const removeButton = target?.closest("[data-remove-index]") as HTMLElement | null;
